...
import com.hillssoft.framework.type.LimitedLinkedList;
...
abstract public class BaseLogger {
	...
	public static class LogInfo {
		...
		public String toString() {
			return String.format("%s %s %s", DateFormat.format("MM-dd kk:mm:ss", date), BaseLogger.toSimpleStringLogLevel(logLevel), message);
		}
	}
	...
	public static String getTraceInfo(StackTraceElement[] stacks) {
		...
		String loggerName = BaseLogger.class.getCanonicalName();

		for (int i = 0; i < stacks.length; i++) {
			if (!stacks[i].getClassName().startsWith(loggerName) && stacks[i].getClassName().startsWith("com.kakao")) {
				stack = stacks[i];
				break;
			}
		}
